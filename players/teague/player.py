...
class Player(Bot):
    ...
    def bet_weight(self):
        ...
        if self.score < 0:
            ...
            # return 1 - average_loss_needed #play mich tighter or solely fold
            return 1
    ...
    def get_action(self, game_state, round_state, active):
        ...
        strength = self.calc_strength(my_cards, board_cards, _MONTE_CARLO_ITERS)*self.bet_weight()


        if street == 3: # flop
            raise_amount = int(my_pip + continue_cost + random.uniform(0.5,0.7)*strength*(pot_total+continue_cost))
        elif street == 4: # turn
            raise_amount = int(my_pip + continue_cost + random.uniform(0.5,0.9)*strength*(pot_total+continue_cost))
        else: # river
            raise_amount = int(my_pip + continue_cost + random.uniform(0.5, 1.0)*strength*(pot_total+continue_cost))
